...
public class ColorTableModel extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object text, boolean isSelected, boolean hasFocus, int row, int column) {
		...
		int valueAsInt = Integer.parseInt(value.split(" ")[0]);
		int minValue = Integer.parseInt(((RealTimeTableModel)table.getModel()).getValueAt(row, 4).toString());
		int maxValue = Integer.parseInt(((RealTimeTableModel)table.getModel()).getValueAt(row, 5).toString());
